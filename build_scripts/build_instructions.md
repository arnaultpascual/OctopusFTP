...
### Custom Icon
Add a custom icon by modifying the build script:
```bash
# Windows
pyinstaller --onefile --windowed --name="OctopusFTP" --icon=path/to/icon.ico main.py

# Mac
pyinstaller --onefile --windowed --name="OctopusFTP" --icon=path/to/icon.icns main.py
```
